...
@section('content')
    <div x-data="scannerApp()" x-init="initApp()"
        class="flex-1 flex flex-col h-screen overflow-hidden bg-slate-900 relative w-full font-outfit">
        <!-- Scanner Header -->
        <header
            class="glass-dark border-b-0 h-20 flex items-center justify-between px-6 shrink-0 z-20 absolute top-0 left-0 right-0">
            ...
            <div class="flex flex-col items-end">
                ...
                <span class="text-xs flex items-center gap-1.5" :class="isOnline ? 'text-emerald-400' : 'text-slate-400'">
                    <span class="w-1.5 h-1.5 rounded-full"
                        :class="isOnline ? 'bg-emerald-500 animate-pulse' : 'bg-slate-500'"></span>
        ...
        <main class="flex-1 relative flex flex-col lg:flex-row mt-20">
            ...
            <div class="flex-1 relative bg-black flex flex-col items-center justify-center overflow-hidden">
                ...
                <div class="absolute inset-0 opacity-10"
                    style="background-image: url('data:image/svg+xml,<svg xmlns=%22http://www.w3.org/2000/svg%22 width=%2240%22 height=%2240%22><rect width=%2240%22 height=%2240%22 fill=%22none%22/><path d=%22M0 0h20v20H0zM20 20h20v20H20z%22 fill=%22%23ffffff%22 fill-opacity=%220.05%22/></svg>');">
                </div>
                ...
                <div class="absolute top-6 left-1/2 -translate-x-1/2 z-30 bg-slate-900/80 p-1 rounded-full flex backdrop-blur-md border border-slate-700 shadow-xl"
                    x-show="isCameraOn">
                    <button @click="setMode('masuk')"
                        class="px-6 py-2 rounded-full text-sm font-bold transition-all tracking-wide"
                        :class="scanMode === 'masuk' ? 'bg-emerald-500 text-white shadow-[0_0_15px_rgba(16,185,129,0.5)]' : 'text-slate-400 hover:text-white'">MASUK</button>
                    <button @click="setMode('pulang')"
                        class="px-6 py-2 rounded-full text-sm font-bold transition-all tracking-wide"
                        :class="scanMode === 'pulang' ? 'bg-indigo-500 text-white shadow-[0_0_15px_rgba(99,102,241,0.5)]' : 'text-slate-400 hover:text-white'">PULANG</button>
                </div>

                <!-- Video Stream Container -->
                <div
                    class="relative w-full h-full max-w-3xl max-h-[75vh] flex items-center justify-center bg-slate-800/50 rounded-2xl border border-slate-700 overflow-hidden shadow-2xl z-10">
                    <!-- If Camera is OFF show this -->
                    <div x-show="!isCameraOn && !isLaunching" class="text-center p-8">
                        <div
                            class="w-24 h-24 bg-slate-900 rounded-full border-4 border-slate-700 flex items-center justify-center mx-auto mb-6 shadow-lg">
                        ...
                        <p class="text-slate-400 text-sm max-w-md mx-auto mb-8">Klik tombol di bawah untuk meluncurkan
                            server Python OpenCV dan mengaktifkan streaming kamera secara langsung.</p>
                    ...
                    <!-- Python MJPEG Stream -->
                    <img x-show="isCameraOn" :src="videoSrc" class="w-full h-full object-cover"
                        x-on:error="handleStreamError" alt="Video Stream">
                    ...
                    <div class="absolute inset-0 pointer-events-none flex items-center justify-center z-20"
                        ...
                        <div class="relative w-72 h-72 sm:w-96 sm:h-96"
                            :class="scanMode === 'masuk' ? 'text-emerald-500' : 'text-indigo-500'">
                            <div class="absolute top-0 left-0 w-12 h-12 border-t-4 border-l-4 rounded-tl-xl"
                                :class="scanMode === 'masuk' ? 'border-emerald-500' : 'border-indigo-500'"></div>
                            <div class="absolute top-0 right-0 w-12 h-12 border-t-4 border-r-4 rounded-tr-xl"
                                :class="scanMode === 'masuk' ? 'border-emerald-500' : 'border-indigo-500'"></div>
                            <div class="absolute bottom-0 left-0 w-12 h-12 border-b-4 border-l-4 rounded-bl-xl"
                                :class="scanMode === 'masuk' ? 'border-emerald-500' : 'border-indigo-500'"></div>
                            <div class="absolute bottom-0 right-0 w-12 h-12 border-b-4 border-r-4 rounded-br-xl"
                                :class="scanMode === 'masuk' ? 'border-emerald-500' : 'border-indigo-500'"></div>

                            <div class="absolute top-0 left-0 w-full h-1 rounded-full animate-[scan_2s_ease-in-out_infinite_alternate]"
                                :class="scanMode === 'masuk' ? 'bg-emerald-400 shadow-[0_0_15px_3px_rgba(52,211,153,0.8)]' : 'bg-indigo-400 shadow-[0_0_15px_3px_rgba(99,102,241,0.8)]'">
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="absolute bottom-6 left-1/2 -translate-x-1/2 z-30" x-show="isCameraOn">
                    <button @click="stopScanner()"
                        class="w-14 h-14 rounded-full bg-rose-600 text-white flex items-center justify-center hover:bg-rose-500 transition-all hover:scale-110 shadow-lg shadow-rose-500/50">
            ...
            <div
                class="w-full lg:w-[400px] glass-dark border-l border-slate-700/50 z-10 flex flex-col relative h-[45vh] lg:h-full lg:rounded-tl-3xl shadow-[-10px_0_30px_rgba(0,0,0,0.5)]">
                ...
                <div class="flex-1 p-6 overflow-y-auto custom-scrollbar flex flex-col relative">
                    ...
                    <div class="w-full bg-slate-800/80 rounded-2xl border p-6 relative overflow-hidden mb-6 shadow-lg transition-colors duration-500"
                        x-show="attendances.length > 0" :class="{
                        ...
                        <div class="absolute -top-20 -right-20 w-40 h-40 rounded-full blur-3xl opacity-30" :class="{
                        ...
                        <div class="relative z-10 flex flex-col items-center text-center">
                            <div class="w-20 h-20 rounded-full bg-slate-700 border-2 p-1 mb-3 relative" :class="{
                                ...
                                <div class="w-full h-full rounded-full bg-slate-700 flex items-center justify-center">
                                    <span class="text-white font-bold text-lg"
                                        x-text="getInitials(attendances[0]?.name)"></span>
                                </div>
                            </div>

                            <h3 class="text-xl font-bold text-white mb-1" x-text="attendances[0]?.name">Nama</h3>
                            <p class="text-slate-300 text-sm mb-4"><span x-text="attendances[0]?.nis"></span> &bull; <span
                                    x-text="attendances[0]?.class_name"></span></p>

                            <div class="w-full bg-slate-900/50 rounded-xl p-3 flex justify-between items-center">
                                <span class="px-3 py-1 rounded-md text-xs font-bold uppercase tracking-wider" :class="{
                                ...
                                <span class="text-sm text-emerald-400 font-mono font-bold"
                                    x-text="attendances[0]?.time">00:00:00</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="space-y-3" x-show="attendances.length > 1">
                        ...
                        <template x-for="(att, index) in attendances.slice(1)" :key="att.id">
                            <div
                                class="flex items-center justify-between p-3 rounded-xl bg-slate-800/30 border border-slate-700/50 hover:bg-slate-800/80 transition-colors">
                                <div class="flex items-center gap-3">
                                    <div
                                        class="w-8 h-8 rounded-full bg-slate-700 flex items-center justify-center shrink-0">
                                        <span class="text-white font-semibold text-xs"
                                            x-text="getInitials(att.name)"></span>
                                    </div>
                                    <div>
                                ...
                                <div class="text-right">
                                    <p class="text-[10px] uppercase font-bold" :class="{
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.data('scannerApp', () => ({
                ...
                initApp() {
                    ...
                    // Shutdown camera when page is closed or navigated away
                    this._beforeUnloadHandler = () => {
                        navigator.sendBeacon('http://127.0.0.1:5000/shutdown');
                    };
                    window.addEventListener('beforeunload', this._beforeUnloadHandler);

                    // Intercept link clicks (e.g. "Kembali ke Beranda") to stop camera before leaving
                    this._linkHandler = (e) => {
                        const link = e.target.closest('a[href]');
                        if (link && this.isCameraOn) {
                            e.preventDefault();
