    ...
    <div class="flex-1 flex flex-col h-screen overflow-hidden bg-slate-50 w-full transition-all duration-300">
        ...
        <main class="flex-1 overflow-y-auto p-6 custom-scrollbar" x-data="{ showStatusModal: false, editRecord: {} }">
                    ...
                    <form method="GET" action="{{ route('attendances.index') }}"
                        ...
                        <div class="w-full md:w-auto flex gap-2">
                            ...
                            <a href="{{ route('attendances.index') }}"
                                class="px-3 py-2 bg-slate-100 text-slate-600 rounded-lg text-sm font-medium hover:bg-slate-200 transition-colors flex items-center justify-center tooltip-trigger"
                                title="Reset Filter">
                ...
                @if(!$selectedClass)
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                        @forelse($classStats as $stat)
                            <a href="{{ route('attendances.index', ['date' => $date, 'class' => $stat['name']]) }}"
                                class="block bg-white p-6 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md hover:border-emerald-200 transition-all group">
                                <div class="flex items-center justify-between mb-4">
                                    <div>
                                        <h3
                                            class="text-lg font-bold text-slate-800 font-outfit group-hover:text-emerald-600 transition-colors">
                                            Kelas {{ $stat['name'] }}</h3>
                                    ...
                                    <div
                                        class="w-10 h-10 rounded-full bg-emerald-50 flex items-center justify-center text-emerald-600 group-hover:bg-emerald-600 group-hover:text-white transition-colors">
                                ...
                                <div class="mb-4">
                                    <div class="flex justify-between text-xs mb-1.5">
                                        ...
                                        <span
                                            class="font-bold text-emerald-600">{{ $stat['total_students'] ? round(($stat['hadir'] / $stat['total_students']) * 100) : 0 }}%</span>
                                    </div>
                                    <div class="w-full bg-slate-100 rounded-full h-2 overflow-hidden flex">
                                        @if($stat['total_students'] > 0)
                                            <div class="bg-emerald-500 h-full transition-all"
                                                style="width: {{ ($stat['hadir'] / $stat['total_students']) * 100 }}%"></div>
                                            <div class="bg-amber-400 h-full transition-all"
                                                style="width: {{ ($stat['izin'] / $stat['total_students']) * 100 }}%"></div>
                                            <div class="bg-blue-500 h-full transition-all"
                                                style="width: {{ ($stat['sakit'] / $stat['total_students']) * 100 }}%"></div>
                                        @endif
                                    </div>
                                </div>
                                ...
                                <div class="grid grid-cols-2 gap-y-3 gap-x-4 text-sm mt-5 pt-5 border-t border-slate-50">
                                    <div class="flex items-center justify-between">
                                        <span class="text-slate-500 flex items-center gap-1.5">
                                            <div class="w-2 h-2 rounded-full bg-emerald-500"></div> Hadir
                                        </span>
                                        <span class="font-semibold text-slate-800">{{ $stat['hadir'] }}</span>
                                    </div>
                                    <div class="flex items-center justify-between">
                                        <span class="text-slate-500 flex items-center gap-1.5">
                                            <div class="w-2 h-2 rounded-full bg-amber-400"></div> Izin
                                        </span>
                                        <span class="font-semibold text-slate-800">{{ $stat['izin'] }}</span>
                                    </div>
                                    <div class="flex items-center justify-between">
                                        <span class="text-slate-500 flex items-center gap-1.5">
                                            <div class="w-2 h-2 rounded-full bg-blue-500"></div> Sakit
                                        </span>
                                        <span class="font-semibold text-slate-800">{{ $stat['sakit'] }}</span>
                                    </div>
                                    <div class="flex items-center justify-between">
                                        <span class="text-slate-500 flex items-center gap-1.5">
                                            <div class="w-2 h-2 rounded-full bg-rose-500"></div> Terlambat
                                        </span>
                    ...
                    <div class="mb-6 flex items-center justify-between">
                        <div>
                            <a href="{{ route('attendances.index', ['date' => $date]) }}"
                                class="inline-flex items-center gap-1 text-sm font-medium text-emerald-600 hover:text-emerald-700 bg-emerald-50 px-3 py-1.5 rounded-lg transition-colors mb-3">
                            ...
                            <p class="text-sm text-slate-500">Menampilkan detail siswa pada tanggal
                                    ...
                                    @forelse ($students as $student)
                                        ...
                                        <tr class="hover:bg-slate-50 transition-colors group">
                                            ...
                                            <td class="px-6 py-4 text-right">
                                                <button
                                                    @click="editRecord = { student_id: {{ $student->id }}, name: '{{ addslashes($student->name) }}', status: '{{ $status }}' }; showStatusModal = true"
            ...
            <div x-show="showStatusModal" x-transition.opacity
                ...
                style="display: none;">
                <div @click.outside="showStatusModal = false" x-transition.scale.duration.200ms
                    ...
                    <div class="px-6 py-5 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
                        ...
                        <button @click="showStatusModal = false" class="text-slate-400 hover:text-slate-600"><i
                    ...
                    <form action="{{ route('attendances.update_status') }}" method="POST" class="p-6 space-y-5">
                        ...
                        <div class="space-y-1.5">
                            ...
                            <select name="status" x-model="editRecord.status"
                                class="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-800 focus:bg-white focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 outline-none transition-all text-sm appearance-none">
                        ...
                        <div class="flex gap-3 pt-4 border-t border-slate-100">
                            ...
                            <button type="submit"
                                class="flex-1 px-4 py-2.5 bg-emerald-600 text-white rounded-xl hover:bg-emerald-700 font-medium text-sm shadow-sm shadow-emerald-600/20 transition-colors">Simpan
                                Status</button>
                        </div>
                    </form>
                </div>
