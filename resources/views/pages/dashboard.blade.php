        ...
        <header class="bg-white border-b border-slate-200 h-20 flex items-center justify-between px-6 shrink-0 z-10 shadow-sm">
            <div class="flex items-center gap-4">
                ...
                <div class="hidden sm:block relative">
                    ...
                    <input type="text" placeholder="Cari data siswa, guru..." class="pl-10 pr-4 py-2 w-64 lg:w-80 rounded-xl bg-slate-100 border-transparent focus:bg-white focus:border-emerald-500 focus:ring-2 focus:ring-emerald-200 transition-all text-sm outline-none">
                </div>
            </div>
            ...
            <div class="flex items-center gap-6">
                ...
                <div class="flex items-center gap-3 cursor-pointer group">
                    ...
                    <div class="w-10 h-10 rounded-full bg-emerald-100 border-2 border-emerald-200 overflow-hidden group-hover:border-emerald-500 transition-colors">
                        <img src="https://ui-avatars.com/api/?name=Admin+Sekolah&background=10b981&color=fff" alt="Admin" class="w-full h-full object-cover">
                    </div>
            ...
            <div class="max-w-7xl mx-auto">
                <div class="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
                    ...
                    <div class="flex items-center gap-3">
                        <button class="flex items-center gap-2 bg-white border border-slate-200 text-slate-600 px-4 py-2 rounded-lg text-sm font-medium hover:bg-slate-50 transition-colors shadow-sm">
                            <i data-lucide="download" class="w-4 h-4"></i>
                            Export
                        </button>
                        <button class="flex items-center gap-2 bg-emerald-600 text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-emerald-700 transition-colors shadow-sm shadow-emerald-600/20">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-8">
                    <x-cards title="Total Siswa" value="{{ number_format($totalStudents) }}" icon="users" color="blue" trend="up" trendValue="+0" />
                    <x-cards title="Hadir" value="{{ number_format($hadir) }}" icon="check-circle" color="emerald" trend="up" trendValue="{{ $totalStudents ? round(($hadir/$totalStudents)*100) : 0 }}%" />
                ...
                <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                    <div class="bg-white p-6 rounded-xl border border-slate-100 shadow-sm lg:col-span-2">
                        <div class="flex justify-between items-center mb-6">
                            ...
                            <select class="bg-slate-50 border border-slate-200 text-slate-600 text-sm rounded-lg px-3 py-1.5 outline-none">
                    ...
                    <div class="bg-white p-6 rounded-xl border border-slate-100 shadow-sm">
                        ...
                        <div class="h-56 w-full flex items-center justify-center relative">
                            ...
                            <div class="absolute inset-0 flex flex-col items-center justify-center pointer-events-none mt-4">
                                <span class="text-3xl font-bold text-slate-800">{{ $totalStudents ? round(($hadir/$totalStudents)*100) : 0 }}%</span>
                                <span class="text-xs text-slate-500">Kehadiran</span>
                            </div>
                        </div>
                        <div class="mt-6 space-y-3">
                            <div class="flex items-center justify-between text-sm">
                                <div class="flex items-center gap-2"><div class="w-3 h-3 rounded-full bg-emerald-500"></div><span class="text-slate-600">Hadir</span></div>
                                <span class="font-medium text-slate-800">{{ number_format($hadir) }}</span>
                            </div>
                            <div class="flex items-center justify-between text-sm">
                                <div class="flex items-center gap-2"><div class="w-3 h-3 rounded-full bg-amber-400"></div><span class="text-slate-600">Izin/Sakit</span></div>
                                <span class="font-medium text-slate-800">{{ number_format($izin + $sakit) }}</span>
                            </div>
                            <div class="flex items-center justify-between text-sm">
                                <div class="flex items-center gap-2"><div class="w-3 h-3 rounded-full bg-rose-500"></div><span class="text-slate-600">Alpha</span></div>
                ...
                <div class="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden mb-8">
                        ...
                        <table class="w-full text-left border-collapse">
                            ...
                            <tbody class="text-sm divide-y divide-slate-100">
                                ...
                                <tr class="hover:bg-slate-50 transition-colors group">
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            <img src="https://ui-avatars.com/api/?name={{ urlencode($attendance->student->name) }}&background=f1f5f9" class="w-10 h-10 rounded-full" alt="{{ $attendance->student->name }}">
                                            <div>
                                                <p class="font-semibold text-slate-800 group-hover:text-emerald-600 transition-colors">{{ $attendance->student->name }}</p>
                                    ...
                                    <td class="px-6 py-4">
                                        ...
                                        <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border {{ $color }} capitalize">
                                            <span class="w-1.5 h-1.5 rounded-full {{ $dotColor }}"></span> {{ $attendance->status }}
                                        </span>
                                    </td>
                                    <td class="px-6 py-4 text-right">
                                        <button class="text-slate-400 hover:text-emerald-600 transition-colors"><i data-lucide="more-vertical" class="w-5 h-5"></i></button>
                                    </td>
                                </tr>
                                @empty
                                <tr>
                                    <td colspan="5" class="px-6 py-8 text-center text-slate-500">Belum ada data scan absensi hari ini.</td>
                                </tr>
                                @endforelse
                            </tbody>
                        </table>
                    ...
                    <div class="p-4 border-t border-slate-100 flex items-center justify-between">
                        ...
                        <div class="flex gap-1">
                            <button class="px-3 py-1 rounded border border-slate-200 text-slate-400 hover:bg-slate-50 disabled:opacity-50" disabled><i data-lucide="chevron-left" class="w-4 h-4"></i></button>
                            <button class="px-3 py-1 rounded bg-emerald-600 text-white font-medium">1</button>
                            <button class="px-3 py-1 rounded border border-slate-200 text-slate-600 hover:bg-slate-50 font-medium">2</button>
                            <button class="px-3 py-1 rounded border border-slate-200 text-slate-600 hover:bg-slate-50 font-medium">3</button>
                            <span class="px-2 py-1 text-slate-400">...</span>
                            <button class="px-3 py-1 rounded border border-slate-200 text-slate-600 hover:bg-slate-50"><i data-lucide="chevron-right" class="w-4 h-4"></i></button>
                        </div>
                    </div>
                </div>
            </div>
