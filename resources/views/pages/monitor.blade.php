...
@section('content')
    <div x-data="monitorApp()" x-init="initMonitor()"
        class="flex-1 flex flex-col h-screen overflow-hidden bg-slate-900 w-full font-outfit relative">

        <!-- Header -->
        <header class="glass-dark border-b-0 h-20 flex items-center justify-between px-6 shrink-0 z-20">
            <div class="flex items-center gap-4">
                <button @click="sidebarOpen = !sidebarOpen"
                    class="md:hidden text-slate-400 hover:text-emerald-400 transition-colors">
            ...
            <div class="flex items-center gap-3">
                ...
                <div class="w-2.5 h-2.5 rounded-full"
                    :class="isOnline ? 'bg-emerald-500 animate-pulse shadow-[0_0_8px_rgba(16,185,129,0.8)]' : 'bg-rose-500'">
                </div>
            </div>
        </header>
        ...
        <main class="flex-1 overflow-y-auto p-6 custom-scrollbar relative">
            <div class="max-w-4xl mx-auto space-y-6">
                ...
                <div class="glass-dark rounded-3xl p-8 border border-emerald-500/30 relative overflow-hidden shadow-2xl transition-all"
                    ...
                    <h2 class="text-sm font-semibold text-slate-400 uppercase tracking-widest mb-6 text-center">Scan
                        Terakhir</h2>

                    <div class="flex flex-col sm:flex-row items-center gap-8 relative z-10">
                        <div
                            class="w-32 h-32 rounded-full border-4 border-slate-700 overflow-hidden shrink-0 shadow-lg bg-slate-700 flex items-center justify-center">
                            <span class="text-white font-bold text-4xl" x-text="getInitials(attendances[0]?.name)"></span>
                        </div>
                        <div class="flex-1 text-center sm:text-left">
                            <h3 class="text-4xl font-bold text-white mb-2" x-text="attendances[0]?.name">Nama Siswa</h3>
                            <p class="text-xl text-slate-300 font-medium mb-4">NIS: <span
                                    x-text="attendances[0]?.nis"></span> &bull; Kelas: <span
                                    x-text="attendances[0]?.class_name"></span></p>

                            <div class="flex flex-wrap items-center justify-center sm:justify-start gap-4">
                                <span class="px-6 py-2 rounded-full text-sm font-bold uppercase tracking-wider" :class="{
                                ...
                                <span class="text-lg font-mono text-white bg-slate-800/80 px-4 py-2 rounded-xl"
                ...
                <div x-show="attendances.length === 0"
                    class="glass-dark rounded-3xl p-12 text-center border border-slate-700/50">
                ...
                <!-- Recent History List -->
                <div class="glass-dark rounded-2xl border border-slate-700/50 overflow-hidden"
                    x-show="attendances.length > 1">
                    ...
                    <div class="divide-y divide-slate-700/50">
                        <template x-for="(att, index) in attendances.slice(1)" :key="att.id">
                            <div class="flex items-center justify-between p-4 hover:bg-slate-800/50 transition-colors">
                                <div class="flex items-center gap-4">
                                    <div
                                        class="w-10 h-10 rounded-full bg-slate-700 flex items-center justify-center shrink-0">
                                        <span class="text-white font-semibold text-sm"
                                            x-text="getInitials(att.name)"></span>
                                    </div>
                                    <div>
                                        <p class="text-white font-medium text-sm" x-text="att.name"></p>
                                        <p class="text-xs text-slate-400"><span x-text="att.class_name"></span> &bull; <span
                                                x-text="att.nis"></span></p>
                                    </div>
                                </div>
                                <div class="flex items-center gap-4 text-right">
                                    <span class="text-xs px-2 py-1 rounded-md uppercase font-bold" :class="{
